...
const setPlayerFn: ((patch: { gameAv: { x: number; y: number } }) => void) | null = null
const getGameAvFn: () => { x: number; y: number } = () => ({ x: 225, y: 235 })
  ...
  if (d < 70) {
    ...
    document.dispatchEvent(
      new CustomEvent('game:toast', {
        detail: '«Минас Аветисян» — история, автор и связи во вселенной',
      })
    )
  }
}
